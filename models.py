...
class CustomNN(nn.Module):
    def __init__(self, backbone, nb_classes, loss, hidden_neurons=1024, dropout=0.4):
        ...
        if backbone == 'resnet':
            self.base_nn = models.resnet50(pretrained=True)
            ...
            self.base_nn = pretrainedmodels.__dict__['se_resnext50_32x4d'](num_classes=1000, pretrained='imagenet')
            trained_kernel = self.base_nn.layer0.conv1.weight
            new_conv = nn.Conv2d(6, 64, kernel_size=7, stride=2, padding=3, bias=False)
            with torch.no_grad():
                new_conv.weight[:,:] = torch.stack([torch.mean(trained_kernel, 1)]*6, dim=1)
            self.base_nn.layer0.conv1 = new_conv
            num_ftrs_cnn = 3*self.base_nn.last_linear.in_features
            self.base_nn.avg_pool = nn.AdaptiveAvgPool2d(output_size=(1,1))
            self.base_nn.last_linear = nn.Identity()
        elif backbone == 'efficientnet':
            self.base_nn = EfficientNet.from_pretrained('efficientnet-b3')
            trained_kernel = self.base_nn._conv_stem.weight
            self.base_nn._conv_stem.in_channels = 6
            self.base_nn._conv_stem.weight = torch.nn.Parameter(torch.stack([torch.mean(trained_kernel, 1)]*6, dim=1))
            num_ftrs_cnn = 3*self.base_nn._fc.in_features
            self.base_nn._bn1 = nn.Identity()
            self.base_nn._fc = nn.Identity()
