...
for path in tqdm(paths):
    channel = int(path.split('_')[2][1])-1
    im = ((cv2.imread(path, cv2.IMREAD_GRAYSCALE)/255)-mean[channel]) / std[channel]
    count_verification[channel] += 1
    sum_x_verification[channel] += np.sum(im)
    sum_x2_verification[channel] += np.sum(im**2)
